...
class WorkflowExecutionService:
    ...
    async def execute_workflow(
        ...
        try:
            # 워크플로우 로드
            graph, vertices, edges = await self.persistence_service.load_workflow(
                graph_id
            )

            # 워크플로우 엔진에 로드
            success = await self.workflow_engine.load_workflow(vertices, edges)
            if not success:
                raise ValueError("워크플로우 로드 실패")

            # 워크플로우 실행
            result = await self.workflow_engine.execute_workflow(initial_inputs)
    ...
    async def get_workflow_status(self, graph_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            await self.workflow_engine.load_workflow(vertices, edges)
